                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="card-box table-responsive">
                                    ...
                                    <div class="col-md-12">
                                        <form action="{{ route('admins.update',$admin) }}" class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" name="active" required>
                                                                <option value="1" {{$admin->active == 1 ? 'selected' : ''}}>@lang('lang.active')</option>
                                                                <option value="0" {{$admin->active == 0 ? 'selected' : ''}}>@lang('lang.un_active')</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row role_level {{$admin->role_id == 2 ? 'd-none' : ''}}">
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            <label for="field-1" class="control-label">@lang('lang.subject')</label>
                                                            <select class="form-control" name="subject" id="subject">
                                                                <option value="">@lang('lang.choose')</option>
                                                                @if($subjects->count() > 0)
                                                                    @foreach ($subjects as $subject)
                                                                        <option value="{{$subject->name_en}}" {{$subject->name_en == $admin->subject ? 'selected' : ''}}>{{$subject->name}}</option>
                                                                    @endforeach
                                                                @endif
                                                            </select>
                                                        </div>
