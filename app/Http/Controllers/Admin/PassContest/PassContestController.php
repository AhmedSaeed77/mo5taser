...
namespace App\Http\Controllers\Admin\PassContest;


use App\Models\Admin;
use App\Models\Category;
use App\Models\PassExam;
use App\Notifications\PassContest;
...
class PassContestController extends Controller
{
    ...
    public function index()
    {
        try
        {
            $categories = Category::category('contest')->get()->pluck('id')->toArray();
            $passes = PassExam::whereIn('level',$categories)->orWhereIn('main_cat',$categories)->get();
            $teachers = Admin::where('role_id',3)->get();
            $main_categories = Category::category('contest')->where('parent_id',NULL)->get();
            return view('dashboard.contests.passed.index',compact('passes','teachers','main_categories'));
        }
        catch(\Exception $ex)
        {
    ...
    public function edit($id)
    {
        try
        {
            ...
            if(isset($pass))
            {
                ...
                return view('dashboard.contests.passed.edit',compact('pass','teachers','main_categories','levels'));
            }
            else
            {
                return back()->with('failed' , __('lang.not_found'));
    ...
    public function store(PassRequest $request)
    {
        try
        {
            ...
            \Notification::send(Admin::where('id', $request->teacher_id)->get(), new PassContest($exam));
    ...
    public function update(PassRequest $request,$id)
    {
        try
        {
            ...
            if(isset($pass))
            {
                ...
                \Notification::send(Admin::where('id', $request->teacher_id)->get(), new PassContest($pass));
                return redirect()->route('pass-contest.index')->with('success' , __('lang.updated'));
            }
            else
            {
                return back()->with('failed' , __('lang.not_found'));
            ...
            if(isset($pass))
            {
                ...
                if($notifications->count() > 0)
                {
                    foreach($notifications as $notify)
                    {
                        ...
                        if($notify->type == 'App\Notifications\PassContest')
                        {
